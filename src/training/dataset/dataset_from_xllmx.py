...
class FinetuneConversationDataset(Dataset):
    def __init__(self, config_path, item_processor, cache_on_disk=False):
        ...
        if self.cache_on_disk:
            ...
            self.length = []
            for meta in self.meta_collection:
              self.length.extend(meta["item_len_list"]) #add by lyt
            print('collect length:',len(self.length))
        else:
            cache_dir = None
    ...
    def _load_meta(self, meta):
        if "type" not in meta:
            meta["type"] = "default"
        if "ratio" not in meta:
            meta_ratio = 1
        else:
            meta_ratio = meta["ratio"]
        ...
        meta["len"] = int(len(annotations)*meta_ratio)
        item_len = int(len(annotations)*meta_ratio)
        ...
        if len(annotations) < item_len:
            ...
            annotations = annotations[:item_len]
        print('after ratio annotations',len(annotations))
        meta["item_len_list"] = [self.item_processor.predict_item_token_length(_) for _ in annotations]
        self.length = meta["item_len_list"] #add by lyt
    ...
    def __getitem__(self, index):
        overall_len = sum([_["len"] for _ in self.meta_collection])
        index = index % overall_len
        import random
        meta_idx, idx_in_meta = self.tie_index_to_meta(index)
        data_dict =  self.get_item_func(meta_idx, idx_in_meta)
        '''while True:
            try:
                meta_idx, idx_in_meta = self.tie_index_to_meta(index)
                data_dict =  self.get_item_func(meta_idx, idx_in_meta)
                ...
                index = random.randint(0, overall_len)'''
