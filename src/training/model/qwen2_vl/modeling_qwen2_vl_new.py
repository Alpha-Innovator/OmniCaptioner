    ...
    def get_rope_index(
        ...
        if image_grid_thw is not None or video_grid_thw is not None:
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                #print('image_nums',image_nums,'video_nums',video_nums, 'token len', len(input_tokens))
                for _ in range(image_nums + video_nums):
                    if image_token_id in input_tokens and remain_images > 0:
                        #print('token len', len(input_tokens),'st',st, 'st index', st < len(input_tokens),'st found image token',image_token_id in input_tokens[st:],'remain_images',remain_images)
                        #print('st index', st < len(input_tokens))
                        #print('st found image token',image_token_id in input_tokens[st:])
                        #print('image_token_id in input_tokens',image_token_id in input_tokens[])
                        ed_image = input_tokens.index(image_token_id, st)
                    else:
                        ed_image = len(input_tokens) + 1 #说明不存在image token
                    ...
                    text_len = ed - st #从上一个image token结束到下一个image token开始
                    ...
                    #print('llm_grid_t ,llm_grid_h ,llm_grid_w',llm_grid_t ,llm_grid_h ,llm_grid_w)
