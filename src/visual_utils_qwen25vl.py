...
def fetch_video(ele: dict, image_factor: int = IMAGE_FACTOR, return_video_sample_fps: bool = False) -> torch.Tensor | list[Image.Image]:
    if isinstance(ele["video"], str):
        ...
        max_pixels = max(video_max_pixels, int(min_pixels * 1.05))
        ...
        sample_fps = nframes / max(video_length, 1e-6) * video_fps
        if return_video_sample_fps:
            return video, sample_fps #2->1
        return video
        #return video
    else:
...
def process_vision_info(
    conversations: list[dict] | list[list[dict]], return_video_kwargs
    ...
    for vision_info in vision_infos:
        if "image" in vision_info or "image_url" in vision_info:
            ...
            #video_inputs.append(fetch_video(vision_info, return_video_sample_fps=True))
